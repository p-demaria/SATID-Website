...
def generate_dashboard_html(dates, portfolio_values, portfolio_fbis,
    ...
    # Use metrics from JSON (fail safe - show N/A if not available)
    if usd_at_risk is not None and pct_to_risk_off is not None:
        usd_distance = usd_at_risk
        distance_pct_display = pct_to_risk_off
    else:
        usd_distance = "N/A"
        distance_pct_display = "N/A"
...
<body>
    ...
    <div class="container">
        <div class="content-page">
            ...
            {f'''
            <div style="background: #e74c3c; color: white; padding: 15px; border-radius: 8px; text-align: center; margin-bottom: 20px; font-weight: 600;">
                ⚠️ WARNING: Risk metrics unavailable - Run Portfolio_Risk_Exposure.py to generate Portfolio_Risk_Metrics.json
            </div>
            ''' if usd_distance == 'N/A' else ''}
                ...
                <div class="metrics-grid">
                    <div class="metric-card">
                        ...
                        <div class="metric-body">
                            <div class="metric-value" style="color: {'#27ae60' if distance_pct_display != 'N/A' and distance_pct_display > 0 else '#e74c3c' if distance_pct_display != 'N/A' else '#95a5a6'};">
                                {distance_pct_display if distance_pct_display == 'N/A' else f'{distance_pct_display:.2f}%'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="metric-card">
                        ...
                        <div class="metric-body">
                            <div class="metric-value" style="color: {'#1e3c72' if usd_distance != 'N/A' else '#95a5a6'};">
                                {usd_distance if usd_distance == 'N/A' else f'${usd_distance:,.0f}'}
                            </div>
                        </div>
                    </div>
                </div>
...
def main():
    ...
    try:
        ...
        print("[2.5/5] Loading portfolio risk metrics...")
        try:
            with open('Portfolio_Risk_Metrics.json', 'r') as f:
                risk_metrics = json.load(f)
            usd_at_risk = risk_metrics['usd_to_support']
            pct_to_risk_off = risk_metrics['pct_to_support']
            print(f"   ✓ USD at Risk: ${usd_at_risk:,.0f}")
            print(f"   ✓ Distance: {pct_to_risk_off:.2f}%")
        except FileNotFoundError:
            print("   ✗ ERROR: Portfolio_Risk_Metrics.json not found")
            print("   → Run Portfolio_Risk_Exposure.py first to generate metrics")
            usd_at_risk = None
            pct_to_risk_off = None
        except Exception as e:
            print(f"   ✗ ERROR reading Portfolio_Risk_Metrics.json: {e}")
            usd_at_risk = None
            pct_to_risk_off = None
