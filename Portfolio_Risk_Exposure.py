...
def main():
    ...
    try:
        ...
        # Save portfolio risk metrics to JSON for Dashboard
        portfolio_total_usd = sum(s['total_usd'] for s in asset_class_summary.values())
        portfolio_total_pct = (portfolio_total_usd / PORTFOLIO_VALUE) * 100

        risk_metrics = {
            'usd_to_support': abs(portfolio_total_usd),  # Absolute value for display
            'pct_to_support': abs(portfolio_total_pct),  # Absolute percentage
            'generated_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        }

        with open('Portfolio_Risk_Metrics.json', 'w') as f:
            json.dump(risk_metrics, f, indent=2)

        print(f"JSON: Portfolio_Risk_Metrics.json")
